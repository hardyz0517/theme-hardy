...
- Moments `stats.upvote` is a read-only counter in the documented theme contract. No public
  ...
  `halo:comment` extension point. The list links to the detail route for comments; only the
  detail template renders the comment widget so list size cannot multiply plugin instances.
- Authenticated Moments publishing uses the UC endpoint
  `/apis/uc.api.moment.halo.run/v1alpha1/moments`; image attachments use the Halo UC upload
  endpoint `/apis/uc.api.storage.halo.run/v1alpha1/attachments/-/upload` with a multipart `file`;
  deleting an owned moment uses `DELETE` on the same Moment endpoint with its metadata name.
